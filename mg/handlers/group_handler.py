...
from models.admin import Group, GroupFunctions,model_to_dict
...
class GroupHandler(BaseHandler):
    def get(self, *args, **kwargs):
        ...
        with DBContext('r') as session:
            if key and value:
                count = session.query(Group).filter(Group.status != '10').filter_by(**{key: value}).count()
                user_info = session.query(Group).filter(Group.status != '10').filter_by(**{key: value}).order_by(
                    Group.dep_id).offset(limit_start).limit(int(limit))
            else:
                count = session.query(Group).filter(Group.status != '10').count()
                user_info = session.query(Group).filter(Group.status != '10').order_by(Group.dep_id).offset(
                    limit_start).limit(int(limit))

            all_user = session.query(Group).filter(Group.status != '10').all()
            if int(limit) > 200:
                user_info = all_user
        ...
        with redis_conn.pipeline(transaction=False) as p:
            ...
            p.execute()
        self.write(dict(code=0, msg='获取用户成功', count=count, data=group_list))
